        ...
        <div className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/8 bg-[#0f0f0f] shadow-2xl shadow-black/60">
          <div className="flex items-center gap-2 border-b border-white/6 px-4 py-3">
            <div className="flex gap-1.5">
              {[
                { cls: "bg-[#ff5f57]", label: "close" },
                { cls: "bg-[#ffbd2e]", label: "minimize" },
                { cls: "bg-[#28c840]", label: "maximize" },
              ].map((b, i) => (
                <div
                  key={i}
                  aria-label={b.label}
                  title={b.label}
                  className={`h-3.5 w-3.5 rounded-full ${b.cls} ring-1 ring-black/20 border border-black/10 shadow-sm`}
                />
              ))}
            </div>
